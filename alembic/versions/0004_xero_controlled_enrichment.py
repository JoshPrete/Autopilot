"""Xero controlled enrichment — audit workflow, review queue, cost history

Revision ID: 0004
Revises: 0003
Create Date: 2026-02-22

1. Adds workflow columns to xero_line_mappings (source, status, approval tracking).
2. Converts legacy string confidence to REAL.
3. Creates xero_review_queue and xero_cost_history tables.
"""
...
def upgrade() -> None:
    # ── xero_line_mappings: add workflow columns ─────────────────────────────
    op.execute("""
    ...
    # Convert legacy TEXT confidence column to REAL if needed
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (
                ...
                UPDATE xero_line_mappings SET confidence_real = CASE
                    ...
                    WHEN confidence ~ '^[0-9]+(\\.[0-9]+)?$'
                         THEN GREATEST(0, LEAST(1, confidence::REAL))
                    ELSE 0.0
                END WHERE confidence_real IS NULL;
    ...
    op.execute("""
        UPDATE xero_line_mappings SET
            source       = COALESCE(NULLIF(source, ''), 'llm'),
    ...
    op.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'xero_line_mappings_source_check') THEN
                ALTER TABLE xero_line_mappings
                    ADD CONSTRAINT xero_line_mappings_source_check
                    CHECK (source IN ('human', 'llm', 'rule'));
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'xero_line_mappings_status_check') THEN
                ALTER TABLE xero_line_mappings
                    ADD CONSTRAINT xero_line_mappings_status_check
                    CHECK (status IN ('proposed', 'approved', 'rejected'));
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'xero_line_mappings_confidence_check') THEN
                ALTER TABLE xero_line_mappings
                    ADD CONSTRAINT xero_line_mappings_confidence_check
                    CHECK (confidence IS NULL OR (confidence >= 0 AND confidence <= 1));
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'xero_line_mappings_units_per_pack_check') THEN
                ALTER TABLE xero_line_mappings
    ...
    op.execute("""
        CREATE TABLE IF NOT EXISTS xero_review_queue (
            ...
            queue_status         TEXT NOT NULL DEFAULT 'open'
                                     CHECK (queue_status IN ('open', 'resolved')),
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS xero_cost_history")
    op.execute("DROP TABLE IF EXISTS xero_review_queue")
    op.execute("""
        ALTER TABLE xero_line_mappings
